...
public class DongACrawling {
	...
	public static void main(String[] args) {
		...
		String html = "";
		try {
			String addr = "https://www.donga.com/news/search?query=" + URLEncoder.encode("박문석", "utf8") + "&x=0&y=0";
		...
		try {
			...
			// 원하는 부분 가져오기
			// #content > div:nth-child(4) > div > h2 > span:nth-child(1)
			Elements elements = document.select("#content > div:nth-child(4) > div > h2 > span:nth-child(1)");
		...
		try (PrintWriter pw = new PrintWriter("./article.txt")) {
			...
			for (String link : articleList) {
				try {
					...
					Document document = Jsoup.parse(html);
					Elements elements = document.select("#content > div.article_view > div.article_txt");
					for (int i = 0; i < elements.size(); i = i + 1) {
						Element element = elements.get(i);
						// 태그 안의 내용을 파일에 기록
					...
					System.out.printf("세부 기사 가져오기 에러:%s\n", e.getMessage());
				}
